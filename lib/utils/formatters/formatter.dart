...
class TFormatter {
  ...
  static String formatPhoneNumber(String phoneNumber) {
    ...
    // Assuming a 13-digit Ng phone number format : (234) 906-770-7890
    if (phoneNumber.length == 10) {
      return '(${phoneNumber.substring(0, 3)}) ${phoneNumber.substring(3, 6)} ${phoneNumber.substring(6)}';
    } else if (phoneNumber.length== 11) {
      return '(${phoneNumber.substring(0, 4)}) ${phoneNumber.substring(4, 7)} ${phoneNumber.substring(7)}';
    } else if (phoneNumber.length ==13) {
      return '(${phoneNumber.substring(1, 4)}) ${phoneNumber.substring(4, 8)} ${phoneNumber.substring(8)}';
    }
    // Add more custom hnone number formatting logic for different formats if needed
    return phoneNumber;
  }
